...
class IkametClient:
    ...
    def _get_captcha_uri(self) -> str:
        ...
        if not captcha_uri:
            raise ValueError('Element with ID=\"CaptchaImage\" does not contain src attr')
        return captcha_uri
    ...
    def _get_captcha_obj(self, captcha_uri: str) -> bytes:
        ...
        if captcha_obj == BAD_CAPTCHA_IMAGE:
            raise ValueError('This is no captcha on the image, it is default \"paywall\"')
        return captcha_obj
    ...
    def _get_captcha_id_by_path(self, captcha_uri: str) -> str:
        ...
        if not captcha_id:
            raise ValueError('Cannot parse captcha ID from the given link')
        return captcha_id[0]
